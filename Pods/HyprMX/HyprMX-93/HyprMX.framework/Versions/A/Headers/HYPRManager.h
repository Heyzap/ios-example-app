 ...
 * 
 * Updating the userId requires a call to [[HYPRManager sharedManager] initialize] for the 
 * change to take effect on future inventory checks.
 *
...
/**
 *  This method will determine if the Inventory Cache is either valid or not valid. In addition, this
 *  method will also check to see if an offer can be displayed. If an offer cannot
 *  be displayed, has been displayed, or the Inventory Cache is invalid then this method will call the
 *  checkInventory: method to refresh the Inventory Cache
 *
 *  @param A block that will simply return the checkInventory callback when the checkInventory: method's
 *         callback is invoked.
 */
- (void)canShowAd:(HYPRBooleanResultCallback)checkCallback;
